...
export function Flow({flow}: FlowContentProps)  {
	...
	useEffect(() => {
		...
		flow?.screens.forEach(screen => {
			const currentWidth = (screen.position_x + 1) * (flow.screens_properties.width + flow.screens_properties.offset_x);
			const currentHeight = (screen.position_y + 1) * (flow.screens_properties.height + flow.screens_properties.offset_y);
